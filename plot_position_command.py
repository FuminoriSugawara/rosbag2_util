...
def plot_position_command_comparison(joint_states_file, commands_file, output_dir='plots'):
    # 出力ディレクトリの作成
    ...
    if not available_joints:
        print("No joint position data found in the CSV file")
        return
    ...
    for joint_number in available_joints:
        position_col = f'joint_{joint_number}_pos'
        command_col = f'command_{joint_number}'
        
        position_min = min(position_min, joint_states_df[position_col].min())
        position_max = max(position_max, joint_states_df[position_col].max())
    ...
    # y軸の範囲を設定（余白を含む）
    y_min = min(position_min, command_min)
    y_max = max(position_max, command_max)
    ...
    for joint_number in available_joints:
        ...
        # グラフの設定
        plt.title(f'Joint {joint_number} Position and Command Comparison')
        plt.xlabel('Time')
        plt.ylabel('Position/Command Value')
        plt.legend()
...
def main():
    ...
    output_dir = 'position_command_plots'  # 出力ディレクトリ名
    
    # 間引きなしの場合
    plot_position_command_comparison(joint_states_file, commands_file, output_dir)
