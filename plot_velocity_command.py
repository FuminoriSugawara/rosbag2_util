...
def plot_velocity_command_comparison(joint_states_file, commands_file, output_dir='plots'):
    # 出力ディレクトリの作成
    ...
    if not available_joints:
        print("No joint velocity data found in the CSV file")
        return
    ...
    for joint_number in available_joints:
        velocity_col = f'joint_{joint_number}_vel'
        command_col = f'command_{joint_number}'
        
        velocity_min = min(velocity_min, joint_states_df[velocity_col].min())
        velocity_max = max(velocity_max, joint_states_df[velocity_col].max())
        
        if command_col in commands_df.columns:
            command_min = min(command_min, commands_df[command_col].min() / 10)
            command_max = max(command_max, commands_df[command_col].max() / 10)
    
    # y軸の範囲を設定（余白を含む）
    y_min = min(velocity_min, command_min)
    y_max = max(velocity_max, command_max)
    ...
    for joint_number in available_joints:
        ...
        # グラフの設定
        plt.title(f'Joint {joint_number} Velocity and Command Comparison')
        plt.xlabel('Time')
        plt.ylabel('Velocity/Command Value')
        plt.legend()
...
def main():
    ...
    output_dir = 'velocity_command_plots'  # 出力ディレクトリ名
    
    # 間引きなしの場合
    plot_velocity_command_comparison(joint_states_file, commands_file, output_dir)
